...
final readonly class ProgressBarDto
{
    public function __construct(
        private InputDto $progressHeight = new InputDto(label: 'Progress Height', type: InputTypeEnum::SIZE, default: '1rem'),
        private InputDto $progressFontSize = new InputDto(label: 'Progress Font Size', type: InputTypeEnum::SIZE, default: '$font-size-base * .75'), // Assuming $font-size-base is a size
        private InputDto $progressBg = new InputDto(label: 'Progress Background', type: InputTypeEnum::COLOR, default: 'var(--#{$prefix}secondary-bg)'), // Assuming it's a color
        private InputDto $progressBorderRadius = new InputDto(label: 'Progress Border Radius', type: InputTypeEnum::SIZE, default: 'var(--#{$prefix}border-radius)'), // Assuming it's a size
        private InputDto $progressBoxShadow = new InputDto(label: 'Progress Box Shadow', type: InputTypeEnum::STRING, default: 'var(--#{$prefix}box-shadow-inset)'), // Assuming it's a string
        ...
        private InputDto $progressBarAnimationTiming = new InputDto(label: 'Progress Bar Animation Timing', type: InputTypeEnum::STRING, default: '1s linear infinite'), // Assuming it's a string
        private InputDto $progressBarTransition = new InputDto(label: 'Progress Bar Transition', type: InputTypeEnum::STRING, default: 'width .6s ease') // Assuming it's a string
    )
    {
    }
